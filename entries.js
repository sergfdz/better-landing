...
const JOURNAL_ENTRIES = [
  {
    ...
    body: [
      ...
      "That shift — from reporting what already happened to acting on what's about to — is the whole point, and it shapes every protection <span class=\"bet\">Bet</span><span class=\"ter\">ter</span> will ship with. Self-exclusion links out to the real, official registry where one exists (Spain's RGIAJ right now), never runs its own; everywhere else it says \"coming soon\" honestly instead of pretending, and shows the real National Council on Problem Gambling helpline for the US in the meantime. Trusted-contact alerts loop in one person you choose, over email, the moment new gambling activity shows up — gently, not as a surveillance report. Notifications carry supportive nudges tuned to what's actually happening in your own account when a risk pattern shows up, never a generic shaming one. And bank monitoring, gambling detection, and risk alerts are always on the moment a bank is connected — reading every transaction against your own history, not some fixed threshold — nothing to configure, just something to disclose.",
